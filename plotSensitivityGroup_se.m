function groupDprime = plotSensitivityGroup_se(save_plots)
...
if make_plots
  ...
  stitle = sprintf('Sensitivity : COMPATIBLE (N=%d)', height(groupDprime));
  title(stitle);
  ...
  stitle = sprintf('Sensitivity : INCOMPATIBLE (N=%d)', height(groupDprime));
  title(stitle);
  xlabel('Presentation Time [ms]')
  ylabel('d''')
