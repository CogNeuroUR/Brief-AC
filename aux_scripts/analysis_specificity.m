...
%% Extract probe type page numbers
% Based on the stimuli names extracted from the experiment log,
% extract the names of the pictures used for probing:
% Two types: (1) action; (2) context
probes_action = [];
probes_context = [];

for i=1:length(stimNames)
  path_parts = strsplit(stimNames{i}, '/');
  % Ex: '.' / 'stimuli' / 'target_office_stamping.jpeg'
  fname = path_parts{3};
  parts_fname = strsplit(fname, '_');
  
  if length(parts_fname) > 1
    if isequal(parts_fname{1}, 'probe')
      
      if ~isequal(parts_fname{2}, 'action')
        parts_probe = strsplit(parts_fname{2}, '-');
        %disp(parts_probe{1});
        if isequal(parts_probe{1}, 'context')
          probes_context = [probes_context, i];
        end
      else
        probes_action = [probes_action, i];
      end
    end
  end
end

%% Extract trials for each probe
% The probe screen is the last page/slide in each trial, therefore, the probe 
% type of the trial can be extracted from its code.
...
for i=1:length(ExpInfo.TrialInfo)
  ...
  if any(probes_action(:) == last_page)
    trials_action(end+1, :) = {...
  ...
  elseif any(probes_context(:) == last_page)
